/*******************************************************************************
* This is the dgvoice package made by bwmarrin but with an added parameter to
...
func PlayAudioFile(v *discordgo.VoiceConnection, filename string, stop <-chan bool, pause <-chan bool) {

	pauseStop := make(chan bool, 1)
	...
	go func() {
		for {
			...
			case pause := <-pause:
				if pause {
					pauseCmd := exec.Command("kill", "-TSTP", strconv.Itoa(run.Process.Pid))
					err = pauseCmd.Run()
					if err != nil {
						log.Println("Pause error: ", err)
					}
				} else {
					pauseCmd := exec.Command("kill", "-CONT", strconv.Itoa(run.Process.Pid))
					err = pauseCmd.Run()
					if err != nil {
						log.Println("Pause error", err)
					}
